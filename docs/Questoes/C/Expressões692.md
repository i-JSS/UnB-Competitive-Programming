...
**Saída**
 ...
 Para cada instância imprima uma linha contendo a letra S se a cadeia é bem definida , ou a letra N caso contrário. Restrições 1 ≤ T ≤ 20. a cadeia de caracteres A tem entre 1 e 100000 caracteres. a cadeia de caracteres A contém apenas caracteres '{', '[', '(', '}', ']' e ')'. Exemplos 
...
bool isBalanced(const char *expressao) {
    char pilha[MAX_SIZE];
    int topo = -1;
    ...
    for(int i = 0; expressao[i] != '\0'; i++) {
        ...
        if(caractereAtual == '(' || caractereAtual == '[' || caractereAtual == '{'){
            if(topo < MAX_SIZE - 1) pilha[++topo] = caractereAtual;
            else return false;

        }else if(caractereAtual == ')' || caractereAtual == ']' || caractereAtual == '}'){
            if (topo >= 0) {
                char caractereTopo = pilha[topo--];
                if ((caractereAtual == ')' && caractereTopo != '(') ||
                    (caractereAtual == ']' && caractereTopo != '[') ||
                    (caractereAtual == '}' && caractereTopo != '{')) {
                    return false;
                }
            }
