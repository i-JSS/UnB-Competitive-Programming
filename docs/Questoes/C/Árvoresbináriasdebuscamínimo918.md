...
# Árvores binárias de busca mínimo

<!-- tabs:start -->

#### **Questão**

 Considere uma árvore binária de busca definida por células Sua tarefa nesse exercício é implementar uma função que procure e retorne o valor mínimo de uma árvore binária de busca com raiz r . Para tanto, você deve submeter um arquivo contendo apenas: 1. Os #include necessários para execução das instruções utilizadas no seu código. 2. A definição da struct no . 3. Uma função de procura do valor mínimo 𝑥 na árvore binária de busca. O protótipo desta função deve ser: 
