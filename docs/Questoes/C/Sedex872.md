...
**Saída**
 ...
 Seu programa deve imprimir uma única linha, contendo a letra 'S' caso a bola de boliche caiba dentro da caixa ou 'N' caso contrário Exemplo 
...
int main() {
    int b, a, l, p;
    scanf("%d %d %d %d", &b, &a, &l, &p);
    if(a<b || l<b || p<b) printf("N\n");
    else printf("S\n");
    return 0;
}
```
