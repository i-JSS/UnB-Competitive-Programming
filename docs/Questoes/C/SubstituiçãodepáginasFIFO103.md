...
int main() {
    int q, n, falhas = 0, temp, paginaAtual;
    cin >> q >> n;
    queue<int> fila;
    unordered_set<int> pagina;
    for(int i = 0; i < n; i++){
        cin >> paginaAtual;
        if (pagina.find(paginaAtual) == pagina.end()) {
            if (fila.size() == q) {
                temp = fila.front();
                fila.pop();
                pagina.erase(temp);
            }
            fila.push(paginaAtual);
            pagina.insert(paginaAtual);
            falhas++;
        }
    }
    cout << falhas << endl;
