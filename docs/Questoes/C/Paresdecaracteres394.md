...
Autor: John L. Gardenghi (adaptado do problema countPairs do codingbat.com) }}
...
void calculaPares(char *str, int indice, int pares){
    if(str[indice] != '\0'){
        if(str[indice] == str[indice+2]) pares++;
        calculaPares(str, indice+1, pares);
    }
    else printf("%d", pares);
}
